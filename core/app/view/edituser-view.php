...
?>
	...
	<div class="row">
		<form role="form" method="post" action="./?action=updateuser"
...
<?php endif;?>
					...
					<label>Unidad</label>
						...
						<option value="">-- SELECCIONE UNIDAD --</option>
                                    <?php foreach($unidades as $unidad):?>
                                        <option
							value="<?=$unidad->id;?>"
							<?php if($unidad->id == $user->unidad){echo "selected";}?>><?=$unidad->nombre;?></option>
                                <?php endforeach;?>
                                </select>
                                <?php endif;?>
                            </div>
				...
				<div class="form-group">
					...
					<p class="text-muted">Solo se modifica la contrase&ntilde;a si el
						campo no esta vacio.</p>
				</div>

				<div class="checkbox">
					<label> <input type="checkbox" name="status"
						<?php if($user->status==1){echo "checked";}?>> Activo
					</label>
				</div>
					...
					<label>Tipo</label>
						...
						<option value="">-- SELECCIONE TIPO --</option>
                                    <?php foreach($cats as $cat):?>
                                        <option value="<?=$cat->id;?>"
							<?php if($cat->id == $user->kind){echo "selected";}?>><?=$cat->name;?></option>
                                <?php endforeach;?>
                                </select>
                                <?php endif;?>
                            </div>
