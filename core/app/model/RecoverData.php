...
class RecoverData
{
    ...
    // partiendo de que ya tenemos creado un objecto RecoverData previamente
    // utilizamos el contexto
    public function update ()
    {
        $sql = "update " . self::$tablename .
                 " set nick=\"$this->nick\",name=\"$this->name\",mail=\"$this->mail\",image=\"$this->image\",password=\"$this->password\",status_id=" .
                 $this->status->id . ",usertype_id=" . $this->usertype->id .
    ...
    public static function getUnUsedByUserId ($id)
    {
        $sql = "select * from " . self::$tablename .
                 " where is_used=0 and user_id=$id";
        $query = Executor::doit($sql);
        return Model::one($query[0], new RecoverData());
    }
